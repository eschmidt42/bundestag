# AUTOGENERATED! DO NOT EDIT! File to edit: nbs/04_poll_clustering.ipynb (unless otherwise specified).

__all__ = ['SpacyTransformer', 'test_cleaned_text', 'get_word_frequencies', 'compare_word_frequencies',
           'test_dense_topic_scores', 'pca_plot_lda_topics']
...
def test_cleaned_text(df:pd.DataFrame, col:str='poll_title_nlp_processed'):
    ...
    assert not mask.any(), f'Unexpectedly found empty entries in `{col}`: \n{df.loc[mask]}'
...
def compare_word_frequencies(df:pd.DataFrame, col0:str, col1:str, topn:int=20,
                             label0:str='before spacy', label1:str='after spacy',
    ...
    print(f'Top {topn} word frequencies for {col0}: \n{get_word_frequencies(tmp, _col0).head(topn)}')
    ...
    fig, ax = plt.subplots()
    bins = np.arange(30)
    sns.histplot(data=tmp, x=wc_col0, alpha=.4, ax=ax,
                 color='blue', label=label0, bins=bins)
    sns.histplot(data=tmp, x=wc_col1, alpha=.4, ax=ax,
                 color='green', label=label1, bins=bins)
    ax.legend()
    ax.set(title=title)
    plt.tight_layout()
    plt.show()
...
# Cell
@patch
def _model_preprocessing(self:SpacyTransformer, documents):
...
@patch
def _fit_lda(self:SpacyTransformer, documents, num_topics:int=10):
    ...
    self.lda_topics = {i: descr for (i, descr) in self.lda_model.print_topics(num_topics=num_topics)}

@patch
def fit(self:SpacyTransformer, documents, mode='lda', **kwargs):
    assert mode in ['lda']
    getattr(self,f'_fit_{mode}')(documents, **kwargs)

# Cell
@patch
def _make_topic_scores_dense(self:SpacyTransformer, scores):
    ...
    ndoc = len(scores)
    logger.debug(f'Densifying list of {ntopic} topic scores to {ndoc}-by-{ntopic} array')
...
@patch
def transform_documents(self:SpacyTransformer, documents:pd.Series, label:str='nlp'):
    ...
    return pd.DataFrame(dense, columns=[f'{label}_dim{i}' for i in range(dense.shape[1])],
                        index=documents.index)

# Cell
def test_dense_topic_scores(dense:np.array):
    mask = dense.sum(axis=1) > 1.
    rows = [i for i,m in enumerate(mask) if m]
    assert not mask.any(), f'Found {mask.sum()} documents with topic scores in total exceeding 1 in rows: {rows}\narray:\n{dense[mask]}'
    mask = dense.sum(axis=1) <= 1e-5
    rows = [i for i,m in enumerate(mask) if m]
    assert not mask.any(), f'Found {mask.sum()} documents with topic scores in total smaller than 1e-5 in rows: {rows}\narray:\n{dense[mask]}'

# Cell
@patch
def transform(self:SpacyTransformer, df:pd.DataFrame, col:str='poll_title_nlp_processed',
              do_test:bool=True, label:str='nlp', return_new_cols:bool=False):
...
# Cell
def pca_plot_lda_topics(df_polls:pd.DataFrame, st:SpacyTransformer, original_text_col:str,
    ...
    df_polls = df_polls.drop(columns=['lda_pca_component_0', 'lda_pca_component_1'], errors='ignore')
    tmp = (pd.DataFrame(X, columns=['lda_pca_component_0', 'lda_pca_component_1'])
           .assign(**{'most_relevant_topic_p': most_relevant_topic_p,
                      'most_relevant_topic_ix': most_relevant_topic,
                      'most_relevant_topic_full': [st.lda_topics[ix] for ix in most_relevant_topic]}))
    df_polls = df_polls.join(tmp)

    fig = px.scatter(data_frame=df_polls, x='lda_pca_component_0', y='lda_pca_component_1',
                     hover_data=[original_text_col, 'most_relevant_topic_p'],
                     color='most_relevant_topic_full', title=f'{original_text_col}-based LDA Model visualised')
